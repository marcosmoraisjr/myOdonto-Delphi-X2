...
Begin
  ...
  if RegistroANS = '338648' then //Fundação Sanepar
    Begin
      GeraCategoriasSanepar(IDConvenio);
    End
  Else if RegistroANS = '' then //Fundação Copel
    Begin
...
Begin
  With dm.qryAux2 do
    Begin
      ...
      FieldByName('DESCRICAO').AsString                   := 'Consulta Odontológica para Avaliação técnica de Auditoria';
      FieldByName('VALOR').AsFloat                        := 33;
      Post;
      FieldByname('ID_CATEGORIAPROCEDIMENTO').AsInteger   := IDCategoria;
      FieldByName('CODPROCEDIMENTO').AsString             := '90010051';
      FieldByName('DESCRICAO').AsString                   := 'Falta Não Justificada';
      FieldByName('VALOR').AsFloat                        := 33;
      Post;
      ...
      if Eof then
        Begin
         // CursorType(crHourGlass);
          ...
          FieldByName('DESCRICAO').AsString := 'Exame Clínico';
          Post;

          frmMensagem.Update;
          frmMensagem.Barra.Properties.Text := 'Gerando: Exame Clínico';
          frmMensagem.Update;
          ExameClinicoSanepar;
          frmMensagem.Update;
