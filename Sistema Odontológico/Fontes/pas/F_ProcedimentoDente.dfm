inherited frmProcedimentoDente: TfrmProcedimentoDente
  ClientHeight = 399
  ClientWidth = 507
  OnActivate = FormActivate
  OnCreate = FormCreate
  ExplicitWidth = 513
  ExplicitHeight = 423
  PixelsPerInch = 96
  TextHeight = 13
  object btnSalvar: TSpeedButton [0]
    Left = 0
    Top = 335
    Width = 143
    Height = 41
    Caption = '&Salvar'
    Flat = True
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    ParentFont = False
    OnClick = btnSalvarClick
  end
  object btnFechar: TSpeedButton [1]
    Left = 362
    Top = 335
    Width = 144
    Height = 41
    Caption = '&Fechar'
    Flat = True
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    ParentFont = False
    OnClick = btnFecharClick
  end
  inherited Panel1: TPanel
    Width = 507
    Caption = 'Procedimento'
    ExplicitWidth = 507
  end
  inherited StatusBar1: TStatusBar
    Top = 375
    Width = 507
    ExplicitTop = 375
    ExplicitWidth = 507
  end
  object GroupBox1: TGroupBox [4]
    Left = 0
    Top = 33
    Width = 507
    Height = 68
    Align = alTop
    TabOrder = 2
    object Label1: TLabel
      Left = 9
      Top = 9
      Width = 72
      Height = 13
      Caption = 'Procedimento :'
      Color = clNavy
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentColor = False
      ParentFont = False
    end
    object EdtCodP: TJvComboEdit
      Left = 85
      Top = 6
      Width = 49
      Height = 21
      Flat = False
      ParentFlat = False
      ButtonWidth = 17
      Color = clWhite
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ImageKind = ikEllipsis
      ParentFont = False
      TabOrder = 0
      Visible = False
      OnExit = EdtCodPExit
    end
    object EdtProcedimento: TEdit
      Left = 86
      Top = 6
      Width = 409
      Height = 21
      Color = clGradientInactiveCaption
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      ReadOnly = True
      TabOrder = 1
    end
    object rgDenteRegiao: TRadioGroup
      Left = 85
      Top = 30
      Width = 185
      Height = 36
      Columns = 2
      ItemIndex = 0
      Items.Strings = (
        'Dente'
        'Regi'#227'o')
      TabOrder = 2
      OnClick = rgDenteRegiaoClick
    end
  end
  object GroupBox2: TGroupBox [5]
    Left = 0
    Top = 101
    Width = 507
    Height = 43
    Align = alTop
    TabOrder = 3
    object lblDente: TLabel
      Left = 9
      Top = 15
      Width = 36
      Height = 13
      Caption = 'Dente :'
    end
    object EdtDente: TEdit
      Left = 52
      Top = 12
      Width = 71
      Height = 21
      Color = clGradientInactiveCaption
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      ReadOnly = True
      TabOrder = 0
    end
    object rgSituação: TRadioGroup
      Left = 168
      Top = 2
      Width = 329
      Height = 37
      Caption = 'Situa'#231#227'o'
      Columns = 4
      ItemIndex = 0
      Items.Strings = (
        'A Relizar'
        'Finalizado'
        'Iniciado'
        'Observado')
      TabOrder = 1
    end
  end
  object PageControl1: TPageControl [6]
    Left = 0
    Top = 144
    Width = 507
    Height = 149
    ActivePage = TabSheet2
    Align = alTop
    TabOrder = 4
    object TabSheet1: TTabSheet
      Caption = 'Observa'#231#245'es'
      ExplicitLeft = 0
      ExplicitTop = 0
      ExplicitWidth = 0
      ExplicitHeight = 0
      object mmoObs: TMemo
        Left = 0
        Top = 0
        Width = 499
        Height = 121
        Align = alClient
        TabOrder = 0
      end
    end
    object TabSheet2: TTabSheet
      Caption = 'TISS - Glosa / Motivo da Reapresenta'#231#227'o'
      ImageIndex = 1
      object GroupBox3: TGroupBox
        Left = 0
        Top = 0
        Width = 499
        Height = 121
        ParentCustomHint = False
        Align = alClient
        BiDiMode = bdLeftToRight
        Color = clBtnFace
        Ctl3D = True
        DoubleBuffered = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentBackground = False
        ParentBiDiMode = False
        ParentColor = False
        ParentCtl3D = False
        ParentDoubleBuffered = False
        ParentFont = False
        ParentShowHint = False
        ShowHint = False
        TabOrder = 0
        object Label3: TLabel
          Left = 173
          Top = 8
          Width = 74
          Height = 13
          Caption = 'Data da Glosa :'
        end
        object mmoTiSS: TMemo
          Left = 8
          Top = 29
          Width = 483
          Height = 86
          TabOrder = 0
        end
        object cbProcGlosado: TCheckBox
          Left = 8
          Top = 6
          Width = 137
          Height = 17
          Caption = 'Procedimento Glosado'
          TabOrder = 1
        end
        object EdtDtGlosa: TJvDateEdit
          Left = 252
          Top = 5
          Width = 93
          Height = 21
          DefaultToday = True
          TabOrder = 2
        end
      end
    end
  end
  object gbRegiao: TGroupBox [7]
    Left = 0
    Top = 293
    Width = 506
    Height = 41
    TabOrder = 5
    object Label4: TLabel
      Left = 9
      Top = 8
      Width = 72
      Height = 13
      Caption = 'TISS - Regi'#227'o :'
    end
    object cbTISS: TComboBox
      Left = 87
      Top = 5
      Width = 362
      Height = 21
      Style = csDropDownList
      ItemIndex = 0
      TabOrder = 0
      Text = '<Nenhuma>'
      Items.Strings = (
        '<Nenhuma>'
        'ASAI - Arcada Superiores e inferiores'
        'AI - Arco Inferior'
        'AS - Arco Superior'
        'AB - Assoalho de boca'
        'CL - Comissura labial'
        'FLI - Freio lingual'
        'FLA - Freios labiais'
        'GI - Gengiva inserida'
        'HAID - Hemi-Arco Inferior Direito'
        'HAIE - Hemi-Arco Inferior Esquerdo'
        'HASD - Hemi-Arco Superior Direito'
        'HASE - Hemi-Arco Superior Esquerdo'
        'LI - L'#225'bio inferior'
        'LS - L'#225'bio Superior'
        'LG - L'#237'ngua'
        'MA - Mucosa alveolar'
        'MJ - Mucosa jugal'
        'PA - Palato'
        'PD - Palato duro'
        'PM - Palato mole'
        'PI - Papila incisiva'
        'PT - Par'#243'tida'
        'PP - Pregas palatinas'
        'RCID - Regi'#227'o de canino inferior direito'
        'RCIE - Regi'#227'o de canino inferior esquerdo'
        'SI - Regi'#227'o de S'#237'nfise'
        'SM - Regi'#227'o do assoalho do seio maxilar'
        'RCSD - Regi'#227'o do canino e lateral superior direito'
        'RCSE - Regi'#227'o do canino e lateral superior esquerdo'
        'TU - Regi'#227'o do T'#250'ber'
        'RIS - Regi'#227'o dos Incisivos centrais superiores'
        'RII - Regi'#227'o dos incisivos inferiores'
        'RMPD - Regi'#227'o dos molares e pr'#233'-molares lado direito'
        'RMPE - Regi'#227'o dos molares e pr'#233'-molares lado esquerdo'
        'RMID - Regi'#227'o dos molares inferiores direito'
        'RMIE - Regi'#227'o dos molares inferiores esquerdo'
        'RMD - Regi'#227'o dos molares lado direito'
        'RME - Regi'#227'o dos molares lado esquerdo'
        'RMSD - Regi'#227'o dos molares superiores direito'
        'RMSE - Regi'#227'o dos molares superiores esquerdo'
        'RPID - Regi'#227'o dos pr'#233'-molares inferiores direito'
        'RPIE - Regi'#227'o dos pr'#233'-molares inferiores esquerdo'
        'RPD - Regi'#227'o dos pr'#233'-molares lado direito'
        'RPE - Regi'#227'o dos pr'#233'-molares lado esquerdo'
        'RCID - Regi'#227'o dos pr'#233'-molares superiores direito'
        'RPSE - Regi'#227'o dos pr'#233'-molares superiores esquerdo'
        'RL - Regi'#227'o lingual'
        'RP - Regi'#227'o palatina'
        'RM - Regi'#227'o retromolar'
        'RSL - Regi'#227'o Sub-Lingual'
        'RSMD - Regi'#227'o Sub-Mandibular Direita'
        'RSME - Regi'#227'o Sub-Mandibular Esquerda'
        'RV - Regi'#227'o vestibular'
        'S5 - Sextante inferior anterior'
        'S6 - Sextante inferior posterior direito'
        'S4 - Sextante inferior posterior esquerdo'
        'S2 - Sextante superior anterior'
        'S1 - Sextante superior posterior direito'
        'S3 - Sextante superior posterior esquerdo'
        'TP - Tonsilas palatinas'
        'UV - '#218'vula')
    end
  end
  object gbFace: TGroupBox [8]
    Left = 0
    Top = 293
    Width = 506
    Height = 41
    TabOrder = 6
    object Label5: TLabel
      Left = 18
      Top = 15
      Width = 62
      Height = 13
      Caption = 'TISS - Face :'
    end
    object cbFace: TComboBox
      Left = 86
      Top = 11
      Width = 362
      Height = 21
      Style = csDropDownList
      ItemIndex = 0
      TabOrder = 0
      Text = '<Nenhuma>'
      Items.Strings = (
        '<Nenhuma>'
        'O - Oclusal'
        'L - Lingual'
        'M - Mesial'
        'V - Vestibular'
        'D - Distal'
        'I - Incisal'
        'P - Palatina')
    end
  end
  inherited Enter: TJvEnterAsTab
    Left = 456
    Top = 0
  end
  object qryAux: TADOQuery
    Connection = DM.Conexao
    CursorType = ctStatic
    Parameters = <>
    Left = 401
  end
  object qryAux2: TADOQuery
    Connection = DM.Conexao
    CursorType = ctStatic
    Parameters = <>
    Left = 33
    Top = 65528
  end
end
