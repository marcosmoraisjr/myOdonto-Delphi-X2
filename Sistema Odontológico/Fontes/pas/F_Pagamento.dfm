inherited frmPagamento: TfrmPagamento
  Caption = '-'
  ClientHeight = 399
  ClientWidth = 675
  OnActivate = FormActivate
  ExplicitWidth = 681
  ExplicitHeight = 423
  PixelsPerInch = 96
  TextHeight = 13
  object btnFechar: TSpeedButton [0]
    Left = 533
    Top = 328
    Width = 142
    Height = 41
    Caption = '&Fechar'
    Flat = True
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    ParentFont = False
    OnClick = btnFecharClick
  end
  object btnSalvar: TSpeedButton [1]
    Left = 0
    Top = 328
    Width = 141
    Height = 41
    Caption = '&Salvar'
    Flat = True
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    ParentFont = False
    OnClick = btnSalvarClick
  end
  object rgCheque: TGroupBox [2]
    Left = 0
    Top = 34
    Width = 487
    Height = 231
    TabOrder = 7
    Visible = False
    object GroupBox7: TGroupBox
      Left = 8
      Top = 8
      Width = 465
      Height = 33
      TabOrder = 0
      object Label16: TLabel
        Left = 12
        Top = 5
        Width = 72
        Height = 19
        Caption = 'Cheque.:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clNavy
        Font.Height = -16
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
    end
    object GroupBox8: TGroupBox
      Left = 8
      Top = 46
      Width = 465
      Height = 161
      Caption = 'Dados'
      TabOrder = 1
      object Label17: TLabel
        Left = 14
        Top = 20
        Width = 36
        Height = 13
        Caption = 'Banco :'
      end
      object Label18: TLabel
        Left = 16
        Top = 47
        Width = 34
        Height = 13
        Caption = 'Nome :'
      end
      object Label19: TLabel
        Left = 10
        Top = 74
        Width = 60
        Height = 13
        Caption = 'Nro. Conta :'
      end
      object Label20: TLabel
        Left = 157
        Top = 74
        Width = 69
        Height = 13
        Caption = 'Nro. Ag'#234'ncia :'
      end
      object Label21: TLabel
        Left = 313
        Top = 74
        Width = 44
        Height = 13
        Caption = 'N'#250'mero :'
      end
      object Label23: TLabel
        Left = 308
        Top = 47
        Width = 49
        Height = 13
        Caption = 'Telefone :'
      end
      object Label24: TLabel
        Left = 19
        Top = 101
        Width = 31
        Height = 13
        Caption = 'Valor :'
      end
      object Label25: TLabel
        Left = 270
        Top = 101
        Width = 87
        Height = 13
        Caption = 'Data Bonifica'#231#227'o :'
      end
      object EdtCodBCP: TJvComboEdit
        Left = 56
        Top = 17
        Width = 49
        Height = 21
        Flat = False
        ParentFlat = False
        ButtonWidth = 17
        ClickKey = 113
        Color = clWhite
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ImageKind = ikEllipsis
        ParentFont = False
        TabOrder = 0
        OnButtonClick = EdtCodBCPButtonClick
        OnExit = EdtCodBCPExit
        OnKeyPress = EdtCodBCPKeyPress
      end
      object EdtBancoCP: TEdit
        Left = 103
        Top = 17
        Width = 194
        Height = 21
        Color = clGradientInactiveCaption
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clNavy
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        ReadOnly = True
        TabOrder = 1
      end
      object EdtNomeCP: TEdit
        Left = 56
        Top = 44
        Width = 241
        Height = 21
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clNavy
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 2
      end
      object EdtNroContaCP: TEdit
        Left = 76
        Top = 71
        Width = 68
        Height = 21
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clNavy
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 4
      end
      object EdtAgenciaCP: TEdit
        Left = 230
        Top = 71
        Width = 67
        Height = 21
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clNavy
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 5
      end
      object EdtNumeroCP: TEdit
        Left = 363
        Top = 71
        Width = 97
        Height = 21
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clNavy
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 6
      end
      object EdtTelCP: TMaskEdit
        Left = 363
        Top = 44
        Width = 93
        Height = 21
        EditMask = '!\(99\) 9999-9999;1;_'
        MaxLength = 14
        TabOrder = 3
        Text = '(  )     -    '
      end
      object EdtValorCP: TcxCurrencyEdit
        Left = 56
        Top = 98
        EditValue = 0c
        ParentFont = False
        Style.Font.Charset = DEFAULT_CHARSET
        Style.Font.Color = clBlack
        Style.Font.Height = -11
        Style.Font.Name = 'Tahoma'
        Style.Font.Style = []
        Style.IsFontAssigned = True
        TabOrder = 7
        OnExit = EdtValorCPExit
        Width = 109
      end
      object EdtDataCP: TJvDateEdit
        Left = 363
        Top = 98
        Width = 97
        Height = 21
        DefaultToday = True
        TabOrder = 8
      end
      object rgCondicao: TRadioGroup
        Left = 300
        Top = 6
        Width = 162
        Height = 38
        Caption = 'Condi'#231#227'o'
        Columns = 2
        ItemIndex = 0
        Items.Strings = (
          #192' Vista'
          'Pr'#233'-Datado')
        TabOrder = 9
      end
    end
  end
  object gbDinheiro: TGroupBox [3]
    Left = 0
    Top = 34
    Width = 487
    Height = 231
    TabOrder = 6
    object GroupBox2: TGroupBox
      Left = 8
      Top = 8
      Width = 465
      Height = 33
      TabOrder = 0
      object Label4: TLabel
        Left = 12
        Top = 5
        Width = 80
        Height = 19
        Caption = 'Dinheiro.:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clNavy
        Font.Height = -16
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
    end
    object EdtVlVista: TcxCurrencyEdit
      Left = 175
      Top = 78
      EditValue = 0c
      ParentFont = False
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clBlack
      Style.Font.Height = -11
      Style.Font.Name = 'Tahoma'
      Style.Font.Style = []
      Style.IsFontAssigned = True
      TabOrder = 1
      OnExit = EdtVlVistaExit
      Width = 109
    end
  end
  object gbChequeRecebido: TGroupBox [4]
    Left = 0
    Top = 34
    Width = 487
    Height = 231
    TabOrder = 8
    Visible = False
    object GroupBox4: TGroupBox
      Left = 8
      Top = 8
      Width = 465
      Height = 33
      TabOrder = 0
      object Label5: TLabel
        Left = 12
        Top = 5
        Width = 168
        Height = 19
        Caption = 'Cheques Recebidos.:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clNavy
        Font.Height = -16
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
    end
    object GroupBox3: TGroupBox
      Left = 8
      Top = 46
      Width = 465
      Height = 178
      TabOrder = 1
      object gridPagamento: TJvDBUltimGrid
        Left = 2
        Top = 15
        Width = 461
        Height = 161
        Align = alClient
        DataSource = dsChequesRecebidos
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        AutoAppend = False
        AutoSort = False
        BeepOnError = False
        SelectColumnsDialogStrings.Caption = 'Select columns'
        SelectColumnsDialogStrings.OK = '&OK'
        SelectColumnsDialogStrings.NoSelectionWarning = 'At least one column must be visible!'
        CanDelete = False
        EditControls = <>
        AutoSizeRows = False
        RowsHeight = 17
        TitleRowHeight = 17
        Columns = <
          item
            Expanded = False
            FieldName = 'Selecionado'
            Title.Caption = 'Sel.'
            Width = 26
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'Historico'
            Title.Caption = 'Hist'#243'rico'
            Width = 162
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DTACONTA'
            Title.Caption = 'Dta. Cheque'
            Width = 70
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DTABOM'
            Title.Caption = 'Dta. Bom.'
            Width = 70
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'Valor'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'Banco'
            Width = 129
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NROAGENCIA'
            Title.Caption = 'N'#186' Agencia'
            Width = 91
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NROCONTA'
            Title.Caption = 'N'#186' Conta'
            Width = 101
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NUMERO'
            Title.Caption = 'N'#186' Cheque'
            Width = 113
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'Correntista'
            Width = 200
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CNPJCPF'
            Title.Caption = 'CPF / CNPJ'
            Width = 95
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'TELEFONE'
            Title.Caption = 'Telefone'
            Width = 131
            Visible = True
          end>
      end
    end
  end
  inherited Panel1: TPanel
    Width = 675
    Caption = 'Forma de Pagamento'
    ExplicitWidth = 675
  end
  inherited StatusBar1: TStatusBar
    Top = 375
    Width = 675
    ExplicitTop = 375
    ExplicitWidth = 675
  end
  object GroupBox1: TGroupBox [7]
    Left = 488
    Top = 34
    Width = 187
    Height = 231
    TabOrder = 2
    object btnDinheiro: TJvXPButton
      Left = 2
      Top = 15
      Width = 183
      Height = 30
      Caption = 'Dinheiro'
      TabOrder = 0
      Down = True
      Align = alTop
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
      Style.Theme = OfficeXP
      OnClick = btnDinheiroClick
    end
    object btnChequeRecebido: TJvXPButton
      Left = 2
      Top = 45
      Width = 183
      Height = 30
      Caption = 'Cheque Recebido'
      TabOrder = 1
      Align = alTop
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      Style.UseStyleManager = False
      OnClick = btnChequeRecebidoClick
    end
    object btnCheque: TJvXPButton
      Left = 2
      Top = 75
      Width = 183
      Height = 30
      Caption = 'Cheque'
      TabOrder = 2
      Align = alTop
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      Style.UseStyleManager = False
      OnClick = btnChequeClick
    end
  end
  object Panel2: TPanel [8]
    Left = 0
    Top = 271
    Width = 225
    Height = 57
    TabOrder = 3
    object Label1: TLabel
      Left = 8
      Top = 17
      Width = 69
      Height = 25
      Caption = 'Total.:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -21
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lblTotal: TLabel
      Left = 173
      Top = 17
      Width = 46
      Height = 25
      Alignment = taRightJustify
      Caption = '0,00'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -21
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
  end
  object Panel3: TPanel [9]
    Left = 225
    Top = 271
    Width = 225
    Height = 57
    TabOrder = 4
    object Label2: TLabel
      Left = 6
      Top = 17
      Width = 110
      Height = 25
      Caption = 'Recebido.:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clGreen
      Font.Height = -21
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lblRecebido: TLabel
      Left = 173
      Top = 17
      Width = 46
      Height = 25
      Alignment = taRightJustify
      Caption = '0,00'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clGreen
      Font.Height = -21
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
  end
  object Panel4: TPanel [10]
    Left = 450
    Top = 271
    Width = 225
    Height = 57
    TabOrder = 5
    object Label3: TLabel
      Left = 6
      Top = 17
      Width = 109
      Height = 25
      Caption = 'Restante.:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clRed
      Font.Height = -21
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lblRestante: TLabel
      Left = 173
      Top = 17
      Width = 46
      Height = 25
      Alignment = taRightJustify
      Caption = '0,00'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clRed
      Font.Height = -21
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
  end
  inherited Enter: TJvEnterAsTab
    Left = 0
    Top = 0
  end
  object qryAux: TADOQuery
    Connection = DM.Conexao
    CursorType = ctStatic
    Parameters = <>
    Left = 104
    Top = 65529
  end
  object qryChequesRecebidos: TADOQuery
    ConnectionString = 
      'Provider=MSDataShape.1;Persist Security Info=True;Data Source='#39#39 +
      ';Data Provider=NONE'
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'SHAPE APPEND'
      'NEW adInteger IDCheque,'
      'NEW AdBoolean Selecionado,'
      'NEW AdInteger IDBancosPadrao,'
      'NEW AdVarChar(100) Banco,'
      'NEW AdVarChar(100) Historico,'
      'NEW AdDouble Valor,'
      'NEW AdDate DTACONTA,'
      'NEW AdDate DTABOM,'
      'NEW AdVarChar(100) NROAGENCIA,'
      'NEW AdVarChar(100) NROCONTA,'
      'NEW AdVarChar(100) NUMERO,'
      'NEW AdVarChar(100) Correntista,'
      'NEW AdVarChar(100) CNPJCPF,'
      'NEW AdVarChar(100) TELEFONE')
    Left = 288
    Top = 177
    object qryChequesRecebidosIDCheque: TIntegerField
      FieldName = 'IDCheque'
    end
    object qryChequesRecebidosSelecionado: TBooleanField
      FieldName = 'Selecionado'
      OnValidate = qryChequesRecebidosSelecionadoValidate
    end
    object qryChequesRecebidosIDBancosPadrao: TIntegerField
      FieldName = 'IDBancosPadrao'
    end
    object qryChequesRecebidosBanco: TStringField
      FieldName = 'Banco'
      Size = 100
    end
    object qryChequesRecebidosHistorico: TStringField
      FieldName = 'Historico'
      Size = 100
    end
    object qryChequesRecebidosValor: TFloatField
      FieldName = 'Valor'
      DisplayFormat = '#,##0.00'
    end
    object qryChequesRecebidosDTACONTA: TDateTimeField
      FieldName = 'DTACONTA'
      DisplayFormat = 'DD/MM/YYYY'
    end
    object qryChequesRecebidosDTABOM: TDateTimeField
      FieldName = 'DTABOM'
      DisplayFormat = 'DD/MM/YYYY'
    end
    object qryChequesRecebidosNROAGENCIA: TStringField
      FieldName = 'NROAGENCIA'
      Size = 100
    end
    object qryChequesRecebidosNROCONTA: TStringField
      FieldName = 'NROCONTA'
      Size = 100
    end
    object qryChequesRecebidosNUMERO: TStringField
      FieldName = 'NUMERO'
      Size = 100
    end
    object qryChequesRecebidosCorrentista: TStringField
      FieldName = 'Correntista'
      Size = 100
    end
    object qryChequesRecebidosCNPJCPF: TStringField
      FieldName = 'CNPJCPF'
      Size = 100
    end
    object qryChequesRecebidosTELEFONE: TStringField
      FieldName = 'TELEFONE'
      Size = 100
    end
  end
  object dsChequesRecebidos: TDataSource
    DataSet = qryChequesRecebidos
    Left = 208
  end
end
