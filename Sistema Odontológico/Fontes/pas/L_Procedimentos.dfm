inherited frmProcedimentosMedico: TfrmProcedimentosMedico
  ClientHeight = 503
  ClientWidth = 1018
  OnActivate = FormActivate
  OnCreate = FormCreate
  ExplicitWidth = 1024
  ExplicitHeight = 527
  PixelsPerInch = 96
  TextHeight = 13
  inherited Panel1: TPanel
    Width = 1018
    Caption = 'Procedimentos'
    ExplicitWidth = 1018
  end
  inherited StatusBar1: TStatusBar
    Top = 479
    Width = 1018
    ExplicitTop = 479
    ExplicitWidth = 1018
  end
  object GroupBox1: TGroupBox [2]
    Left = 0
    Top = 33
    Width = 1018
    Height = 40
    Align = alTop
    TabOrder = 2
    object Label1: TLabel
      Left = 13
      Top = 11
      Width = 89
      Height = 13
      Caption = 'Tabela de Pre'#231'os :'
    end
    object SpeedButton3: TSpeedButton
      Left = 404
      Top = 6
      Width = 115
      Height = 26
      Caption = 'Nova Categoria'
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      ParentFont = False
      OnClick = SpeedButton3Click
    end
    object SpeedButton1: TSpeedButton
      Left = 525
      Top = 6
      Width = 146
      Height = 26
      Caption = 'Novo Procedimento'
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      ParentFont = False
      OnClick = SpeedButton1Click
    end
    object SpeedButton2: TSpeedButton
      Left = 260
      Top = 6
      Width = 135
      Height = 26
      Caption = 'Nova Tab. de Pre'#231'o'
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      ParentFont = False
      OnClick = SpeedButton2Click
    end
    object cbTabela: TComboBox
      Left = 116
      Top = 8
      Width = 132
      Height = 21
      TabOrder = 0
      Text = 'cbTabela'
      OnClick = cbTabelaClick
    end
  end
  object GroupBox2: TGroupBox [3]
    Left = 0
    Top = 73
    Width = 1018
    Height = 363
    Align = alTop
    TabOrder = 3
    object GroupBox3: TGroupBox
      Left = 3
      Top = 6
      Width = 302
      Height = 259
      Caption = 'Categorias'
      TabOrder = 0
      object gListaCategoria: TJvDBGrid
        Left = 2
        Top = 15
        Width = 298
        Height = 242
        Align = alClient
        Ctl3D = True
        DataSource = dsCategorias
        ImeMode = imClose
        Options = [dgTitles, dgIndicator, dgColumnResize, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
        ParentCtl3D = False
        ReadOnly = True
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        OnCellClick = gListaCategoriaCellClick
        OnKeyDown = gListaCategoriaKeyDown
        OnKeyUp = gListaCategoriaKeyUp
        AutoAppend = False
        AutoSort = False
        BeepOnError = False
        AlternateRowColor = clSkyBlue
        AlternateRowFontColor = clBlack
        PostOnEnterKey = True
        SelectColumnsDialogStrings.Caption = 'Select columns'
        SelectColumnsDialogStrings.OK = '&OK'
        SelectColumnsDialogStrings.NoSelectionWarning = 'At least one column must be visible!'
        EditControls = <>
        AutoSizeRows = False
        RowsHeight = 17
        TitleRowHeight = 17
        Columns = <
          item
            Expanded = False
            FieldName = 'DESCRICAO'
            Title.Caption = 'Descri'#231#227'o'
            Width = 266
            Visible = True
          end>
      end
    end
    object GroupBox4: TGroupBox
      Left = 317
      Top = 6
      Width = 692
      Height = 259
      Caption = 'Procedimentos'
      TabOrder = 1
      object gListaProcedimentos: TJvDBGrid
        Left = 2
        Top = 15
        Width = 688
        Height = 242
        Align = alClient
        Ctl3D = True
        DataSource = dsProcedimentos
        ImeMode = imClose
        Options = [dgTitles, dgIndicator, dgColumnResize, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
        ParentCtl3D = False
        ReadOnly = True
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        OnCellClick = gListaProcedimentosCellClick
        OnKeyDown = gListaProcedimentosKeyDown
        OnKeyUp = gListaProcedimentosKeyUp
        AutoAppend = False
        AutoSort = False
        BeepOnError = False
        AlternateRowColor = clSkyBlue
        AlternateRowFontColor = clBlack
        PostOnEnterKey = True
        SelectColumnsDialogStrings.Caption = 'Select columns'
        SelectColumnsDialogStrings.OK = '&OK'
        SelectColumnsDialogStrings.NoSelectionWarning = 'At least one column must be visible!'
        EditControls = <>
        AutoSizeRows = False
        RowsHeight = 17
        TitleRowHeight = 17
        Columns = <
          item
            Expanded = False
            FieldName = 'DESCRICAO'
            ReadOnly = False
            Title.Caption = 'Descri'#231#227'o'
            Width = 657
            Visible = True
          end>
      end
    end
    object GroupBox5: TGroupBox
      Left = 2
      Top = 267
      Width = 1014
      Height = 94
      Align = alBottom
      TabOrder = 2
      object Label2: TLabel
        Left = 11
        Top = 9
        Width = 72
        Height = 13
        Caption = 'Procedimento :'
      end
      object Label3: TLabel
        Left = 748
        Top = 9
        Width = 123
        Height = 13
        Caption = 'C'#243'digo do Procedimento :'
      end
      object Label4: TLabel
        Left = 52
        Top = 36
        Width = 31
        Height = 13
        Caption = 'Valor :'
      end
      object Label5: TLabel
        Left = 219
        Top = 36
        Width = 134
        Height = 13
        Caption = 'Tempo m'#233'dio para concluir :'
      end
      object Label9: TLabel
        Left = 11
        Top = 63
        Width = 202
        Height = 13
        Caption = 'TISS ( Sele'#231#227'o Procedimento / Despesa ) :'
      end
      object EdtProcedimento: TEdit
        Left = 89
        Top = 6
        Width = 632
        Height = 21
        TabOrder = 0
        OnExit = EdtProcedimentoExit
      end
      object EdtCodigoProcedimento: TEdit
        Left = 875
        Top = 6
        Width = 130
        Height = 21
        TabOrder = 1
        OnExit = EdtCodigoProcedimentoExit
      end
      object EdtValor: TcxCurrencyEdit
        Left = 89
        Top = 33
        ParentFont = False
        Style.Font.Charset = DEFAULT_CHARSET
        Style.Font.Color = clBlack
        Style.Font.Height = -11
        Style.Font.Name = 'Tahoma'
        Style.Font.Style = []
        Style.IsFontAssigned = True
        TabOrder = 2
        OnExit = EdtValorExit
        Width = 109
      end
      object cbSelecaoTISS: TComboBox
        Left = 218
        Top = 60
        Width = 142
        Height = 21
        Style = csDropDownList
        ItemIndex = 0
        TabOrder = 4
        Text = 'Procedimentos'
        OnChange = cbSelecaoTISSChange
        OnClick = cbSelecaoTISSClick
        Items.Strings = (
          'Procedimentos'
          'Outras Despesas')
      end
      object cbOutrasDespesasTISS: TComboBox
        Left = 369
        Top = 60
        Width = 109
        Height = 21
        Style = csDropDownList
        TabOrder = 5
        Visible = False
        OnClick = cbOutrasDespesasTISSClick
        Items.Strings = (
          'Gases Medicinais'
          'Medicamentos'
          'Materiais'
          'Taxas Diversas'
          'Di'#225'rias'
          'Aluguel')
      end
      object EdtHora: TJvDateTimePicker
        Left = 359
        Top = 33
        Width = 93
        Height = 21
        Date = 41160.000000000000000000
        Time = 41160.000000000000000000
        Kind = dtkTime
        TabOrder = 3
        OnExit = EdtHoraExit
        DropDownDate = 41160.000000000000000000
      end
    end
  end
  object btnSalvar: TcxButton [4]
    Left = 0
    Top = 435
    Width = 136
    Height = 45
    Caption = '&Salvar'
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    LookAndFeel.Kind = lfFlat
    LookAndFeel.NativeStyle = False
    SpeedButtonOptions.AllowAllUp = True
    SpeedButtonOptions.Flat = True
    TabOrder = 4
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    OnClick = btnSalvarClick
  end
  object btnFechar: TcxButton [5]
    Left = 882
    Top = 435
    Width = 136
    Height = 45
    Caption = '&Fechar'
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    LookAndFeel.Kind = lfFlat
    LookAndFeel.NativeStyle = False
    SpeedButtonOptions.AllowAllUp = True
    SpeedButtonOptions.Flat = True
    TabOrder = 5
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    OnClick = btnFecharClick
  end
  inherited Enter: TJvEnterAsTab
    Left = 112
    Top = 65512
  end
  object dsCategorias: TDataSource
    DataSet = qryCategorias
    Left = 183
    Top = 197
  end
  object dsProcedimentos: TDataSource
    DataSet = qryProcedimentos
    Left = 431
    Top = 181
  end
  object dsTabela: TDataSource
    DataSet = qryTabela
    Left = 63
    Top = 189
  end
  object qryTabela: TADOQuery
    Connection = DM.Conexao
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'SELECT ID_CONVENIO, NOME FROM CONVENIOS')
    Left = 64
    Top = 137
    object qryTabelaID_CONVENIO: TAutoIncField
      FieldName = 'ID_CONVENIO'
      ReadOnly = True
    end
    object qryTabelaNOME: TStringField
      FieldName = 'NOME'
      Size = 255
    end
  end
  object qryCategorias: TADOQuery
    Connection = DM.Conexao
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'SELECT * FROM CATEGORIAPROCEDIMENTO')
    Left = 184
    Top = 145
    object qryCategoriasDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Size = 200
    end
    object qryCategoriasID_CATEGORIAPROCEDIMENTO: TAutoIncField
      FieldName = 'ID_CATEGORIAPROCEDIMENTO'
      ReadOnly = True
    end
    object qryCategoriasID_CONVENIO: TIntegerField
      FieldName = 'ID_CONVENIO'
    end
  end
  object qryProcedimentos: TADOQuery
    Connection = DM.Conexao
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'SELECT * FROM PROCEDIMENTOS')
    Left = 432
    Top = 137
    object qryProcedimentosID_PROCEDIMENTOS: TAutoIncField
      FieldName = 'ID_PROCEDIMENTOS'
      ReadOnly = True
      OnChange = qryProcedimentosID_PROCEDIMENTOSChange
    end
    object qryProcedimentosID_CATEGORIAPROCEDIMENTO: TIntegerField
      FieldName = 'ID_CATEGORIAPROCEDIMENTO'
    end
    object qryProcedimentosDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Size = 200
    end
    object qryProcedimentosCODPROCEDIMENTO: TStringField
      FieldName = 'CODPROCEDIMENTO'
      Size = 100
    end
    object qryProcedimentosVALOR: TFloatField
      FieldName = 'VALOR'
    end
    object qryProcedimentosTEMPOMEDIO: TStringField
      FieldName = 'TEMPOMEDIO'
      Size = 5
    end
    object qryProcedimentosTIPOCOMISSAO: TIntegerField
      FieldName = 'TIPOCOMISSAO'
    end
    object qryProcedimentosVALORCOMISSAO: TFloatField
      FieldName = 'VALORCOMISSAO'
    end
    object qryProcedimentosDESPESASCOMPROTETICOS: TFloatField
      FieldName = 'DESPESASCOMPROTETICOS'
    end
    object qryProcedimentosSELECAOTISS: TIntegerField
      FieldName = 'SELECAOTISS'
    end
    object qryProcedimentosDESPESATISS: TIntegerField
      FieldName = 'DESPESATISS'
    end
  end
  object qryAux: TADOQuery
    Connection = DM.Conexao
    CursorType = ctStatic
    Parameters = <>
    Left = 304
    Top = 137
  end
end
