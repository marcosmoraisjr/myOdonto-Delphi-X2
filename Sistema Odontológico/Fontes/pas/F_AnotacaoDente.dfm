inherited frmAnotacaoDente: TfrmAnotacaoDente
  ClientHeight = 359
  ClientWidth = 466
  OnActivate = FormActivate
  ExplicitWidth = 472
  ExplicitHeight = 383
  PixelsPerInch = 96
  TextHeight = 13
  object btnFechar: TSpeedButton [0]
    Left = 321
    Top = 295
    Width = 145
    Height = 41
    Caption = '&Fechar'
    Flat = True
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    ParentFont = False
    OnClick = btnFecharClick
  end
  object btnSalvar: TSpeedButton [1]
    Left = 0
    Top = 295
    Width = 144
    Height = 41
    Caption = '&Salvar'
    Flat = True
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    ParentFont = False
    OnClick = btnSalvarClick
  end
  inherited Panel1: TPanel
    Width = 466
    Caption = 'Anota'#231#245'es - Dente 11'
    ExplicitWidth = 466
  end
  inherited StatusBar1: TStatusBar
    Top = 335
    Width = 466
    ExplicitTop = 335
    ExplicitWidth = 466
  end
  object GroupBox1: TGroupBox [4]
    Left = 0
    Top = 33
    Width = 466
    Height = 256
    Align = alTop
    TabOrder = 2
    object GroupBox2: TGroupBox
      Left = 2
      Top = 15
      Width = 255
      Height = 239
      Align = alLeft
      Caption = 'Anamalia / Patologia'
      TabOrder = 0
      object cbAnomalia: TCheckListBox
        Left = 2
        Top = 15
        Width = 251
        Height = 222
        Align = alClient
        Items.Strings = (
          '<TISS> - Ausente'
          '<TISS> - Cariado'
          '<TISS> - Extra'#231#227'o indicada'
          '<TISS> - H'#237'gido'
          '<TISS> - Restaurado'
          'Abras'#227'o'
          'Abscesso Periapical Agudo'
          'Abscesso Periapical Cr'#244'nico'
          'Ameloblastoma'
          'Amelog'#234'neses imperfeita'
          'Anodontia'
          'Anodontia parcial'
          'Anguilose'
          'Atri'#231#227'o'
          'Avuls'#227'o'
          'Carcinoma Epiderm'#243'ide'
          'CarcinomaMetast'#225'tico'
          'Cementoblastoma ou cementoma verdadeiro'
          'Cisto de Erup'#231#227'o'
          'Cisto de Reten'#231#227'o Mucoso'
          'Cisto de Stafne'
          'Cisto Dent'#237'gero'
          'Cisto Dem'#243'ide'
          'Cisto do Conduto Nasopalatino'
          'Cisto Mandibular Mediano'
          'Cisto Nasoalveolar'
          'Cisto Odontog'#234'nico Calcificante '
          'Cisto '#211'sseo Aneurism'#225'tico'
          'Cisto '#211'sseo Simples'
          'Cisto '#211'sseo Traum'#225'tico'
          'Cisto Palatino Mediano'
          'Cisto Periodontal Laterla'
          'Cisto Radicular'
          'Cisto Residual'
          'Concuss'#227'o'
          'Condroma'
          'Condrossarcoma'
          'Dens in Dente'
          'Dentes supranumer'#225'rios'
          'Denti'#231#227'o pr'#233'-dec'#237'dua'
          'Dentinog'#234'nese imperfeita'
          'Denstrui'#231#227'o '#211'ssea na doen'#231'a periodontal'
          'Disjun'#231#227'o craniofacial Le Fort III'
          'Displasia cement'#225'ria periapical'
          'Displasia cleidocranial'
          'Displasia cleidocraniana'
          'Displasia ectod'#233'rmica heredit'#225'ria'
          'Displasia fibrosa'
          'Displasia '#211'ssea Florida'
          'Enfermidade de Paget(oste'#237'te deformante)'
          'Fibroma amelobl'#225'stico'
          'Fibrossarcoma'
          'Fratura de corpo mandibula'
          'Fraturas coron'#225'rias'
          'Fraturas corono-radiculares'
          'Fraturas de c'#244'ndilo'
          'Fraturas dento-cr'#226'nio-faciais'
          'Fraturas do rebordo alveolar'
          'Fraturas Le Fort I'
          'Fraturas Le Fort II'
          'Fraturas radiculares'
          'Fraturas radiculares verticais'
          'Fraturas zigom'#225'ticas'
          'Fus'#227'o'
          'Gemina'#231#227'o'
          'Granuloma Central de C'#233'lulas Gigantes'
          'Granuloma eosin'#243'filo'
          'Granuloma Perfiapical'
          'Hemangioma Central'
          'Hiperostose Cortical Infantil'
          'Leucemia'
          'Linfoma de Burkitt'
          'Luxa'#231#227'o'
          'Macrodontia isolada'
          'Melanoma'
          'Microdontia isolada'
          'Mieloma m'#250'tiplo'
          'Mixoma Odontog'#234'nico'
          'Neurilenoma'
          'Neurinoma'
          'Neurofibroma'
          'Odontoma'
          'Osteoblastoma benigno'
          'Osteoma'
          'Osteoma oste'#243'ide'
          'Osteomielite de Garr'#233' / osteomilelite proliferativa'
          'Osteomielite Esclerosante Cr'#244'nica Focal'
          'Osteomielite Esclerosa Difusa'
          'Osteomielite Supurativa Aguda'
          'Osteomielite Supurativa Cr'#244'nica'
          'Osteopetrose'
          'Osteossarcoma'
          'Pedras pulpares'
          'Periodontite do Adulto'
          'Periodontite Juvenil'
          'Queratocisto Odontog'#234'nico(Cisto Primordial)'
          'Querubismo'
          'Raiz supranumer'#225'ria ou acess'#243'ria'
          'Reabsor'#231#227'o externa'
          'Reabsor'#231#227'o interna'
          'Sarcoma de Ewing'
          'S'#237'ndrome de Nevos de C'#233'lulas Basais'
          'Taurodontismo'
          'Transposi'#231#227'o'
          'Tumor Odontog'#234'nico Adenomat'#243'ide'
          'Tumor Odontog'#234'nico Epitelial Calcificante'
          'Tumor Odontog'#234'nico Escamoso'
          'Tumores malignos das gl'#226'ndulas salivares')
        Style = lbOwnerDrawFixed
        TabOrder = 0
      end
    end
    object GroupBox3: TGroupBox
      Left = 261
      Top = 15
      Width = 203
      Height = 239
      Align = alRight
      Caption = 'Anota'#231#227'o'
      TabOrder = 1
      object mmoObs: TMemo
        Left = 2
        Top = 15
        Width = 199
        Height = 222
        Align = alClient
        TabOrder = 0
      end
    end
  end
  inherited Enter: TJvEnterAsTab
    Left = 32
    Top = 65520
  end
  object qryAux: TADOQuery
    Connection = DM.Conexao
    CursorType = ctStatic
    Parameters = <>
    Left = 492
    Top = 36
  end
end
