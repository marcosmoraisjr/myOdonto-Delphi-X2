inherited frmHistoricoCheque: TfrmHistoricoCheque
  ClientHeight = 414
  ClientWidth = 796
  ExplicitWidth = 802
  ExplicitHeight = 438
  PixelsPerInch = 96
  TextHeight = 13
  inherited Panel1: TPanel
    Width = 796
    ExplicitWidth = 796
  end
  inherited StatusBar1: TStatusBar
    Top = 390
    Width = 796
    ExplicitTop = 390
    ExplicitWidth = 796
  end
  object groupbox1: TGroupBox [2]
    Left = 0
    Top = 33
    Width = 796
    Height = 312
    Align = alTop
    TabOrder = 2
    object gridPagamento: TJvDBUltimGrid
      Left = 2
      Top = 15
      Width = 792
      Height = 295
      Align = alClient
      DataSource = dsHistoricoCheque
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'Tahoma'
      TitleFont.Style = []
      AutoAppend = False
      AutoSort = False
      BeepOnError = False
      SelectColumnsDialogStrings.Caption = 'Select columns'
      SelectColumnsDialogStrings.OK = '&OK'
      SelectColumnsDialogStrings.NoSelectionWarning = 'At least one column must be visible!'
      CanDelete = False
      CancelOnMouse = True
      EditControls = <>
      AutoSizeRows = False
      RowsHeight = 17
      TitleRowHeight = 17
      Columns = <
        item
          Expanded = False
          FieldName = 'HISTORICO'
          Title.Caption = 'Hist'#243'rico'
          Width = 509
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'DATA'
          Title.Caption = 'Data'
          Width = 114
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'NOME'
          Title.Caption = 'Operador'
          Width = 133
          Visible = True
        end>
    end
  end
  object btnFechar: TcxButton [3]
    Left = 660
    Top = 344
    Width = 136
    Height = 47
    Caption = '&Fechar'
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    LookAndFeel.Kind = lfFlat
    LookAndFeel.NativeStyle = False
    SpeedButtonOptions.AllowAllUp = True
    SpeedButtonOptions.Flat = True
    TabOrder = 3
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    OnClick = btnFecharClick
  end
  inherited Enter: TJvEnterAsTab
    Left = 168
    Top = 0
  end
  inherited Acoes: TPopupActionBar
    Left = 248
    Top = 65520
  end
  object qryHistoricoCheque: TADOQuery
    Connection = DM.Conexao
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      
        'SELECT HISTORICOCHEQUE.ID_HISTORICOCHEQUE, HISTORICOCHEQUE.ID_CH' +
        'EQUESRECEBIDO, HISTORICOCHEQUE.ID_OPERADOR, CHEQUESRECEBIDO.NUME' +
        'RO,'
      
        'HISTORICOCHEQUE.ID_FILIAL, HISTORICOCHEQUE.DATA, HISTORICOCHEQUE' +
        '.HISTORICO, OPERADORES.NOME FROM HISTORICOCHEQUE '
      
        'INNER JOIN OPERADORES ON HISTORICOCHEQUE.ID_OPERADOR = OPERADORE' +
        'S.ID_OPERADORES'
      
        'INNER JOIN CHEQUESRECEBIDO ON HISTORICOCHEQUE.ID_CHEQUESRECEBIDO' +
        ' = CHEQUESRECEBIDO.ID_CHEQUESRECEBIDO')
    Left = 536
    Top = 65529
    object qryHistoricoChequeHISTORICO: TMemoField
      FieldName = 'HISTORICO'
      BlobType = ftMemo
    end
    object qryHistoricoChequeDATA: TDateTimeField
      FieldName = 'DATA'
      DisplayFormat = 'DD/MM/YYYY HH:MM:SS'
    end
    object qryHistoricoChequeNOME: TStringField
      FieldName = 'NOME'
      Size = 30
    end
    object qryHistoricoChequeID_HISTORICOCHEQUE: TAutoIncField
      FieldName = 'ID_HISTORICOCHEQUE'
      ReadOnly = True
    end
    object qryHistoricoChequeID_CHEQUESRECEBIDO: TIntegerField
      FieldName = 'ID_CHEQUESRECEBIDO'
    end
    object qryHistoricoChequeID_OPERADOR: TIntegerField
      FieldName = 'ID_OPERADOR'
    end
    object qryHistoricoChequeNUMERO: TStringField
      FieldName = 'NUMERO'
      Size = 100
    end
    object qryHistoricoChequeID_FILIAL: TIntegerField
      FieldName = 'ID_FILIAL'
    end
  end
  object dsHistoricoCheque: TDataSource
    DataSet = qryHistoricoCheque
    Left = 480
    Top = 65528
  end
end
