...
begin
  ...
  if (EdtValorCP.Value > 0) And (EdtCodBCP.Text = '') then
    Begin
      application.MessageBox('Para Pagamentos com Cheque Pré-Datado o campo "Banco" é obrigatório o preenchimento!','Erro!!!',mb_ok+MB_ICONERROR);
      Exit;
    End;

  if (EdtValorCP.Value > 0) And (EdtNomeCP.Text = '') then
    Begin
      application.MessageBox('Para Pagamentos com Cheque Pré-Datado o campo "Nome" é obrigatório o preenchimento!','Erro!!!',mb_ok+MB_ICONERROR);
      Exit;
    End;

  if (EdtValorCA.Value > 0) And (EdtCodBCA.Text = '') then
    Begin
      application.MessageBox('Para Pagamentos com Cheque à Vista o campo "Banco" é obrigatório o preenchimento!','Erro!!!',mb_ok+MB_ICONERROR);
      Exit;
    End;

  if (EdtValorCA.Value > 0) And (EdtNomeCA.Text = '') then
    Begin
      application.MessageBox('Para Pagamentos com Cheque à Vista o campo "Nome" é obrigatório o preenchimento!','Erro!!!',mb_ok+MB_ICONERROR);
      Exit;
    End;
...
begin
  ...
  if (EdtVlVista.Value = 0) and (EdtValorCP.Value > 0) And (EdtValorCA.Value = 0) then
    Begin
      i := 29;     //Cheque Pré-datado
    End;

  if (EdtVlVista.Value > 0) and (EdtValorCP.Value = 0) And (EdtValorCA.Value > 0) then
    Begin
      i := 30;     //Dinheiro e Cheque à Vista
    End;

  if (EdtVlVista.Value > 0) and (EdtValorCP.Value > 0) And (EdtValorCA.Value = 0) then
    Begin
      i := 31;     //Dinheiro e Cheque Pré-datado
    End;

  if (EdtVlVista.Value = 0) and (EdtValorCP.Value > 0) And (EdtValorCA.Value > 0) then
    Begin
      i := 32;     //Cheque à Vista e Cheque Pré-Datado
    End;

  if (EdtVlVista.Value > 0) and (EdtValorCP.Value > 0) And (EdtValorCA.Value > 0) then
    Begin
      i := 33;     //Dinheiro, Cheque à Vista e Cheque Pré-Datado
    End;
  Result := i;
end;
